...
describe('Mangakakalot Tests', function () {
    ...
    it("Testing Home-Page aquisition", async() => {
        ...
        expect(homePages[0], "No popular manga section available").to.exist
        expect(homePages[1], "No latest manga section available").to.exist
        expect(homePages[2], "No hot manga section available").to.exist
        expect(homePages[3], "No new manga section available").to.exist
        expect(homePages[4], "No completed manga section available").to.exist
    })

    it("Testing view more", async () => {
        let data = await wrapper.getViewMoreItems(source, 'latest_updates', {page: null}) 
        expect(data, "No server response").to.exist;
        expect(data, "Empty server response").to.not.be.empty;
    })
